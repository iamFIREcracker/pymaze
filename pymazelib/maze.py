...
class Cell(object):
  ...
  def __str__(self):
    ...
    """
    i = 0
    for (n, valid) in enumerate(reversed(self.walls)):
      i += int(valid) * (2 ** n)
    return CELL[i]
...
class Maze(object):
  ...
  def reset(self):
    """Reset each cell in order to delete the current solution generated by a
    solver.

    """
    for row in self.grid:
      for cell in row:
        cell.directions = [not valid for valid in cell.walls]
        cell.carved = False
        cell.active = False
        cell.backward = False
